...
def ensure_required_columns(df, defaults):
    out = df.copy()
...
def norm_round(s):
    ...
    series = pd.Series(s, copy=False).astype(str).str.strip()
...
def detect_team_metadata_columns(teams_metadata_df):
...
# AUTOSUB/CHIPS LOGIC IN UDFs
def apply_autosub(df): # Autosub
    points_series = df.get("points_pairs_str", pd.Series("DID NOT PLAY", index=df.index))
    ...
    df["sub_order"] = pd.to_numeric(df["sub_order"], errors="coerce").fillna(999).astype(int)
...
def apply_captain_vice(df): # Captain/vice-captain
    out = df.copy()
...
def apply_best11(df): # Best11 chip
...
def apply_best11_multiplier(df): # Best11 highest-scorer 2x
    out = df.copy()
    out["fantasy_points_final"] = pd.to_numeric(out["fantasy_points_final"], errors="coerce").fillna(0)
    out["final_playing_xi"] = pd.to_numeric(out.get("final_playing_xi", out["is_playing_xi"]), errors="coerce").fillna(0).astype(int)
    out["points_multiplier"] = pd.to_numeric(out.get("points_multiplier", 1), errors="coerce").fillna(1)
...
def apply_flexicap(df): # Flexicap chip
    out = df.copy()
    out["fantasy_points_final"] = pd.to_numeric(out["fantasy_points_final"], errors="coerce").fillna(0)
    out["final_playing_xi"] = pd.to_numeric(out.get("final_playing_xi", out["is_playing_xi"]), errors="coerce").fillna(0).astype(int)
    out["points_multiplier"] = pd.to_numeric(out.get("points_multiplier", 1), errors="coerce").fillna(1)
...
def apply_matchwinner(df): # Matchwinner chip
    out = df.copy()
    out["points_multiplier"] = pd.to_numeric(out.get("points_multiplier", 1), errors="coerce").fillna(1)
    out["final_playing_xi"] = pd.to_numeric(out.get("final_playing_xi", out["is_playing_xi"]), errors="coerce").fillna(0).astype(int)
    ...
    for (_, _), sub_df in out.groupby(["participant_group", "round"], sort=False):
        ...
        out.loc[mw_idx, "matchwinner_multiplier"] = 3 if out.loc[mw_idx, "potm_bonus"] > 0 else 1
...
# 0 points and points_pairs_str="DID NOT PLAY" which is correct contest behaviour.
...
def is_abandoned_or_no_result_text(value) -> bool:
...
participants_match_df["points_pairs_str"] = participants_match_df["points_pairs_str"].fillna("DID NOT PLAY")
...
def apply_chips(participants_match_df):
...
points_pairs = participants_match_df.get("points_pairs_str", pd.Series("", index=participants_match_df.index)).astype(str).str.strip().str.upper()
...
if missing_required_match_id.any():
    ...
    logger.error(
        ...
        participants_match_df.loc[
            ...
            ["participant_group", "round", "player_name", "team_code", "team_round", "played_match_flag", "points_pairs_str"],
...
columns_to_print = ['participant_group', 'round', 'player_name', 'team', 'team_code', 'position', 'effective_position', 'is_playing_xi', 'sub_order', 
                    ...
                    'chip_converter', 'chip_best11', 'chip_flexicap', 'chip_matchwinner', 'points_pairs_str']
